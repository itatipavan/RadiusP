...
const Applications = () => {
  ...
  useEffect(() => { filterApplications(); }, [searchText, statusFilter, applications]);
  ...
  const filterApplications = () => {
    ...
    if (searchText) {
      filtered = filtered.filter(app => app.studentName.toLowerCase().includes(searchText.toLowerCase()) || app.university.toLowerCase().includes(searchText.toLowerCase()) || app.program.toLowerCase().includes(searchText.toLowerCase()) || app.id.toLowerCase().includes(searchText.toLowerCase()));
    }
    if (statusFilter !== 'all') { filtered = filtered.filter(app => app.status === statusFilter); }
    setFilteredApplications(filtered);
  };

  const getStatusColor = (status) => ({ 'Inquiry Received':'default','Document Review':'orange','Application Submitted':'blue','University Review':'purple','Decision Received':'cyan','Visa Processing':'gold','Visa Approved':'green','Enrolled':'success' }[status] || 'default');

  const columns = [
    { title: 'Application ID', dataIndex: 'id', key: 'id', render: (text, record) => (<div><Text strong style={{ color: '#1976d2' }}>{text}</Text><br /><Text type="secondary" style={{ fontSize: '11px' }}>{record.applicationDate}</Text></div>) },
    { title: 'Student', dataIndex: 'studentName', key: 'student', render: (text, record) => (<Space><Avatar style={{ backgroundColor: '#1976d2' }}>{text.charAt(0)}</Avatar><div><Text strong>{text}</Text><br /><Text type="secondary" style={{ fontSize: '12px' }}>{record.counselor}</Text></div></Space>) },
    { title: 'University & Program', key: 'program', render: (_, record) => (<div><Text strong style={{ fontSize: '13px' }}>{record.university}</Text><br /><Text style={{ fontSize: '12px' }}>{record.program}</Text><br /><Space><Tag color="blue" style={{ fontSize: '10px' }}>{record.level}</Tag><Tag color="green" style={{ fontSize: '10px' }}>{record.country}</Tag></Space></div>) },
    { title: 'Status', dataIndex: 'status', key: 'status', render: (status) => <Tag color={getStatusColor(status)}>{status}</Tag> },
    { title: 'Actions', key: 'actions', render: () => (<Space direction="vertical" size="small"><Button type="link" icon={<EyeOutlined />} size="small">View Details</Button></Space>) },
  ];

  const statusCounts = { total: filteredApplications.length, inProgress: filteredApplications.filter(app => !['Enrolled', 'Rejected'].includes(app.status)).length, completed: filteredApplications.filter(app => app.status === 'Enrolled').length };
