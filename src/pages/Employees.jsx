...
const Employees = () => {
  ...
  useEffect(() => { filterEmployees(); }, [searchText, employees]);
  ...
  const filterEmployees = () => {
    ...
    if (searchText) {
      filtered = filtered.filter(emp => emp.name.toLowerCase().includes(searchText.toLowerCase()) || emp.email.toLowerCase().includes(searchText.toLowerCase()) || emp.role.toLowerCase().includes(searchText.toLowerCase()));
    }
    setFilteredEmployees(filtered);
  };

  const columns = [
    { title: 'Employee', dataIndex: 'name', key: 'name', render: (text, record) => (<Space><Avatar style={{ backgroundColor: '#1976d2' }}>{text.charAt(0)}</Avatar><div><Text strong>{text}</Text><br /><Text type="secondary" style={{ fontSize: '12px' }}>{record.email}</Text></div></Space>) },
    { title: 'Role', dataIndex: 'role', key: 'role', render: (role) => <Tag color="blue">{role}</Tag> },
    { title: 'Department', dataIndex: 'department', key: 'department' },
    { title: 'Experience', dataIndex: 'experience', key: 'experience' },
    { title: 'Success Rate', dataIndex: 'successRate', key: 'successRate', render: (rate) => rate !== 'N/A' ? <Tag color="success">{rate}</Tag> : <Text type="secondary">N/A</Text> },
