...
const Students = () => {
  ...
  const handleAddStudent = async (values) => {
    try {
      const counselors = userStorage
        .getUsers()
        .filter((u) => u.role === "counselor");
      const assignedCounselor = values.assignedCounselor
        ? counselors.find((c) => c.name === values.assignedCounselor)
        : null;
      const studentData = {
        ...values,
        fullName: `${values.firstName} ${values.lastName}`,
        counselorId:
          user.role === "admin" ? assignedCounselor?.id || user.id : user.id,
        assignedCounselor:
          user.role === "admin"
  ...
  const handleEditStudent = async (values) => {
    try {
      ...
      const updatedData = {
        ...
        counselorId:
          user.role === "admin"
            ? assignedCounselor?.id || editingStudent.counselorId
            : editingStudent.counselorId,
        assignedCounselor:
          user.role === "admin"
      ...
      const success = studentStorage.updateStudent(
        editingStudent.id,
        updatedData
      );
      if (success) {
  ...
  const openEditModal = (student) => {
    ...
    form.setFieldsValue({
      ...
      graduationYear: student.graduationYear
        ? dayjs(student.graduationYear, "YYYY")
        : null,
    });
  ...
  const submitRemark = async () => {
    try {
      ...
      const history = Array.isArray(remarkStudent.remarkHistory)
        ? remarkStudent.remarkHistory
        : [];
      history.push({
        userId: user.id,
        userName: user.name,
      ...
      const success = studentStorage.updateStudent(remarkStudent.id, {
        remarkHistory: history,
        lastActivity: new Date().toISOString().split("T")[0],
      });
      if (success) {
        messageApi.success("Remark added");
        setIsRemarkModalVisible(false);
        setRemarkStudent(null);
        loadStudents();
      } else {
        messageApi.error("Failed to add remark");
      }
    } catch {
      /* validation failed */
    }
  ...
  const actionColumn = {
    ...
    render: (_, record) => (
      <Space>
        ...
        {(user.role === "employee" || user.role === "counselor") && (
          <Button
            type="link"
  ...
  const columns =
    user.role === "employee" ? employeeColumns : [...baseColumns, actionColumn];
  ...
  return (
    ...
    >
      ...
      <Modal
        title={`Add Remark for ${remarkStudent?.fullName || ""}`}
